...
internal sealed class WindowsProcessExtensions : IProcessExtensions
{
    ...
    {
        nint handle = Kernel32.OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, false, processId);
        if (handle == nint.Zero)
        ...
        try
        {
            if (Environment.Is64BitOperatingSystem && !Environment.Is64BitProcess) // are we running in WOW?
            {
                ...
                int hr = NtDll.NtWow64QueryInformationProcess64(handle, 0, ref pbi, Marshal.SizeOf(pbi), nint.Zero);
                if (hr != 0)
                    throw new Win32Exception(hr);

                long pp = 0;
                hr = NtDll.NtWow64ReadVirtualMemory64(handle, pbi.PebBaseAddress + processParametersOffset, ref pp, Marshal.SizeOf(pp), nint.Zero);
                if (hr != 0)
                    throw new Win32Exception(hr);

                UNICODE_STRING_WOW64 us = new UNICODE_STRING_WOW64();
                hr = NtDll.NtWow64ReadVirtualMemory64(handle, pp + offset, ref us, Marshal.SizeOf(us), nint.Zero);
                if (hr != 0)
                    throw new Win32Exception(hr);
                ...
                hr = NtDll.NtWow64ReadVirtualMemory64(handle, us.Buffer, s, us.Length, nint.Zero);
                if (hr != 0)
                    throw new Win32Exception(hr);
            ...
            else // we are running with the same bitness as the OS, 32 or 64
            {
                ...
                int hr = NtDll.NtQueryInformationProcess(handle, 0, ref pbi, Marshal.SizeOf(pbi), nint.Zero);
                if (hr != 0)
                    throw new Win32Exception(hr);

                nint pp = new nint();
                if (!Kernel32.ReadProcessMemory(handle, pbi.PebBaseAddress + processParametersOffset, ref pp, new nint(Marshal.SizeOf(pp)), nint.Zero))
                    throw new Win32Exception(Marshal.GetLastWin32Error());

                UNICODE_STRING us = new UNICODE_STRING();
                if (!Kernel32.ReadProcessMemory(handle, pp + offset, ref us, new nint(Marshal.SizeOf(us)), nint.Zero))
                    throw new Win32Exception(Marshal.GetLastWin32Error());

                if (us.Buffer == nint.Zero || us.Length == 0)
                    return string.Empty;

                string s = new('\0', us.Length / 2);
                if (!Kernel32.ReadProcessMemory(handle, us.Buffer, s, new nint(us.Length), nint.Zero))
                    throw new Win32Exception(Marshal.GetLastWin32Error());
