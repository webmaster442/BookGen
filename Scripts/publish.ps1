...
dotnet restore BookGen.sln
dotnet build --no-restore --configuration Release BookGen.sln
dotnet pack --no-build --configuration Release BookGen.sln

dotnet publish --no-restore --configuration Release --output bin\iso\bookgen BookGen.sln

cd bin\iso\bookgen
./bookgen.exe md2html -i ..\..\..\Changelog.md -o ..\changelog.html
cd ..
echo [autorun] > .\autorun.inf
echo open=bookgen\BookGen.Launch.exe >> .\autorun.inf
echo icon=bookgen\BookGen.Launch.exe >> .\autorun.inf
echo bookgen\BookGen.Launch.exe | out-file -Encoding ASCII launch.cmd
cd ..
get-childitem .\iso | New-IsoFile -path .\bookgen.iso
cd ..
